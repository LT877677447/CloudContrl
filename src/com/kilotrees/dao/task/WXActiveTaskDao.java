...
public class WXActiveTaskDao implements ITaskDao{
	private static Logger log = Logger.getLogger(WXActiveTaskDao.class);
	...
	private static TaskWinXinActive assembleQQActiveModelFromSQL(Connection connection, String sql) {
		TaskWinXinActive model = null;
		PreparedStatement ps = null;
		try {
			ps = connection.prepareStatement(sql);
	...
	private static TaskWinXinActive assembleQQActiveModelFromResultSet(ResultSet rs) {
		TaskWinXinActive model = null;
		try {
			if (rs.next()) {
	...
	public static Integer SuccActive(String phoneNumber) {
		int num = 0;
		String sql = "update " + tableName + " set lastLoginTime = ?,status=4,comment='活跃成功(不再释放)' where phoneNumber=?";
		Connection connection = connectionmgr.getInstance().getConnection();
		PreparedStatement ps = null;
		try {
			ps = connection.prepareStatement(sql);
	...
	public static Integer failActive(String phoneNumber) {
		int num = 0;
		String sql = "update " + tableName + " set status=5,comment='活跃失败(不再释放)' where phoneNumber=?";
		Connection connection = connectionmgr.getInstance().getConnection();
		PreparedStatement ps = null;
		try {
			ps = connection.prepareStatement(sql);
